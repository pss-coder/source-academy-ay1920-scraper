        ...
        while True:
            ...
            try:
                ...
                if len(driver.find_elements_by_xpath("//div[@class='ace_line']")) > 1:
                    file = subfolder + "/Task " + str(question_number) + " Template.js"
                    f = open(file, "w", encoding="utf-8")
                    lines = []
                    # Reset text editor to get template
                ...
                try:
                    ...
                    if flag:
                        # Wait for Reset dialog to disappear
                        time.sleep(DIALOG_WAIT)
                    next_button.click()
                    if flag:
...
driver.get(URL)
# Login
login_button = driver.find_element_by_xpath("//button[contains(., 'LumiNUS')]")
login_button.click()
user_id_input = driver.find_element_by_id("username")
user_id_input.send_keys(user_id)
password_input = driver.find_element_by_id("password")
password_input.send_keys(password)
signin_button = driver.find_element_by_id("guestSignIn")
signin_button.click()
